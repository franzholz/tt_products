 ...
 */
use JambageCom\TtProducts\Utility\MoveItemsUtility;
...
class MoveItemsWizardModuleFunctionController
{
    ...
    public function main()
    {
        ...
        if ($execute) {
            $moveItems = GeneralUtility::makeInstance(MoveItemsUtility::class);
            ...
            if (
                isset($_REQUEST['id']) &&
                isset($_REQUEST['move']) &&
                is_array($_REQUEST['move']) &&
                isset($_REQUEST['move']['destination']) &&
                isset($_REQUEST['move']['age'])
            ) {
                $moveResult =
                    $moveItems->moveAll(
                        $infoArray,
                        $_REQUEST['id'],
                        $_REQUEST['move']['destination'],
                        $_REQUEST['move']['age']
                    );
            }
            ...
            if (
                ...
                foreach ($tableArray as $table) {
                    if (
                        ...
                        $information .= sprintf($GLOBALS['LANG']->getLL('moved_table'), intval($rows[$table]), $tableOut);
                        $information .= '<br>';
                    }
                }
                if ($information == '') {
                    $information = $GLOBALS['LANG']->getLL('nothing_moved');
                }
            } else {
                $information = $GLOBALS['LANG']->getLL('nothing');
            }
            $assigns['information'] = $information;
        } else {
            // CSH
            $assigns['cshItem'] = BackendUtility::cshItem('_MOD_web_func', 'tt_products.func1');
            $assigns['pid_src'] = $_REQUEST['id'];

            $view->setTemplatePathAndFilename(GeneralUtility::getFileAbsFileName(
                'EXT:' . TT_PRODUCTS_EXT . '/Resources/Private/Templates/MoveItemsWizard.html'
            ));
        }
